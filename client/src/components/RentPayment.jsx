        ...
        <Grid container spacing={1} sx={{ mb: 3 }}>
          <Grid item xs={12} sm={6} md={2.4}>
            <Card sx={{
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 0.5 }}>
                <Money sx={{ color: 'primary.main', mr: 1, fontSize: 20 }} />
          ...
          <Grid item xs={12} sm={6} md={2.4}>
            <Card sx={{
              ...
              <Typography variant="h5" sx={{ mt: 0.5, fontWeight: 600 }}>
                ${filteredRows.filter(row => row.status === 'Pending').reduce((sum, row) => sum + row.amount, 0).toLocaleString()}
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5, fontSize: '0.85rem' }}>
          ...
          <Grid item xs={12} sm={6} md={2.4}>
            <Card sx={{
              ...
              <Typography variant="h5" sx={{ mt: 0.5, fontWeight: 600 }}>
                ${filteredRows.filter(row => row.status === 'Overdue').reduce((sum, row) => sum + row.amount, 0).toLocaleString()}
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5, fontSize: '0.85rem' }}>
          ...
          <Grid item xs={12} sm={6} md={2.4}>
            <Card sx={{
              ...
              <Typography variant="h5" sx={{ mt: 0.5, fontWeight: 600 }}>
                {filteredRows.length > 0 ? Math.round(filteredRows.filter(row => row.status === 'Paid').length / filteredRows.length * 100) : 0}%
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5, fontSize: '0.85rem' }}>
                {filteredRows.filter(row => row.status === 'Paid').length} paid payments
              </Typography>
